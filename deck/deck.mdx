...
const [input] = docuemnt.getElementsByTagName('input');
...
- 0 kb on the client. You need it only on the server
...
```jsx 90,99,108 file=./05a.jsx title="Extract the constraint of each form control"
```
</Step>
...
```diff 4:19 title="Extract the constraint of each form control"
```
</Step>
